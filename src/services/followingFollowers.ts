...
export const Addfollowing = async (
  ...
  try {
    ...
    await firebase
      ...
      .collection('users')
      .doc(LoggedInUserId)
      .update({ ...user, following: [followingUserId, ...user.following!] });
    ...
    await firebase
      ...
      .update({
        ...followedUser,
        followers: [LoggedInUserId, ...followedUser.followers!],
      });
...
export const SubtractFollowing = async (
  LoggedInUserId: string,
  followingUserId: string
) => {
  try {
    ...
    await firebase
      ...
      .collection('users')
      .doc(LoggedInUserId)
      .update({ ...user, following: newFollowing });
    const followedUser = await getUserByUserId(followingUserId);
    ...
    await firebase
      ...
      .collection('users')
      .doc(followingUserId)
      .update({ ...followedUser, followers: newFollowers });
    console.log('successfully Updated followers');
  } catch (e) {
    console.error(e);
  }
...
export const AlreadyFollowing = async (
  LoggedInUserId: string,
  followingUserId: string
): Promise<boolean> => {
...
export const ToggleFollowing = async (
  ...
  const AreadyFollowing = await AlreadyFollowing(
    LoggedInUserId,
    followingUserId
  );
  if (AreadyFollowing) {
